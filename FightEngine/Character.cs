...
namespace Lecture19Composition
{
	class Character
	{
		...
		public Character(Controller controller,  int maxHp, int attack, int defense, string name = "random")
		{
			...
			string souhlasky = "BCDFGHJKLMNPQRSTVWXZ";
			string namee = "";
            for (int i = 0; i < rnd.Next(2, 4); i++)
				{
                int random_choice1 = rnd.Next(0, souhlasky.Length - 1);
                namee = namee + souhlasky[random_choice1];
				int random_choice2 = rnd.Next(0, samohlasky.Length - 1);
                namee = namee + samohlasky[random_choice2];
				this.name = namee;
				}
            }
            else
